#!/usr/bin/env python3
"""
Folk Medicine Clean Background Generator

Скрипт для генерации ЧИСТЫХ фонов народной медицины без текста и QR.
...
# Подключаем ваши новые сценарии из модуля оверлея
...
class CleanFolkGenerator:
    """
    Генератор чистых изображений на основе SimpleImagePipeline.
    """
    ...
    def load(self):
        ...
        # Импорт стандартного пайплайна вашего проекта
    ...
    def generate(
        ...
        neg = NEG_PROMPT_PERSON if scenario.get("has_person") else NEG_PROMPT
    ...
    def mass_generate(
        ...
        scenarios = scenarios or FOLK_MEDICINE_SCENARIOS
        
        print(f"Начинаю массовую генерацию {count} чистых фонов народной медицины...")
        ...
        for i in range(count):
            ...
            filename = f"folk_clean_{i:04d}_{scenario['name']}_{timestamp}.png"
...
def main():
    parser = argparse.ArgumentParser(description="Clean Folk Medicine Background Generator")
    ...
    parser.add_argument("--output", type=str, default="output/folk_clean_backgrounds")
    
    args = parser.parse_args()
    
    if args.list_scenarios:
        print("\n=== Доступные сценарии народной медицины ===")
        print("\n[БЕЗ людей]:")
